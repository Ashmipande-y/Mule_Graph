...
Threshold selection uses validation data only, never test data (per the
Stage 2 brief). Test metrics are reported once, at the end, using the
threshold chosen on validation -- they are not used to pick or adjust
anything.
"""
...
def compute_report(y_true, y_prob, threshold: float) -> ClassificationReport:
    ...
    if support_positive == 0 or support_negative == 0:
        # Precision/recall/F1 are conventionally undefined (or trivially 0)
        # without both classes present; report honestly rather than hiding it.
